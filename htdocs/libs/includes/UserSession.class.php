    ...
    public static function authenticate($user, $pass, $fingerprint = null)
    {
        ...
        if ($username) {
            ...
            try {
                ...
                throw new Exception("Err in UserSession: " . $e);
            }
        } else {
            return false;
        }
    ...
    */
                ...
                if ($session->isValid() and $session->isActive()) {
                    if ($_SERVER['REMOTE_ADDR'] == $session->getIP()) {
                        if ($_SERVER['HTTP_USER_AGENT'] == $session->getUserAgent()) {
                            if (isset($_SESSION['fingerprint']) && $session->getFingerprint() == $_SESSION['fingerprint']) { //TODO: This is always true, fix it
    ...
    public function isValid()
    {
        if (isset($this->data['login_time'])) {
            ...
            if (86400 > time() - $login_time->getTimestamp()) {
                return true;
            } else {
                return false;
            }
